...
class MaterialController extends Controller
{
    ...
    private function validated(Request $request,BrandContext $context,?Material $material=null):array{$data=$request->validate(['product_code'=>['required','max:100','regex:/^[\p{Han}A-Za-z0-9-]+$/u',Rule::unique('materials')->where(fn($q)=>$q->where('brand_id',$context->id()))->ignore($material?->id)],'name'=>'required|string|max:160','slug'=>['required','alpha_dash','max:160',Rule::unique('materials')->where(fn($q)=>$q->where('brand_id',$context->id()))->ignore($material?->id)],'description'=>'nullable|string|max:50000','price'=>'required|numeric|min:0|max:99999999','stock_on_hand'=>'required|integer|min:0|max:100000','low_stock_threshold'=>'required|integer|min:0|max:100000','status'=>'required|in:draft,published','seo_title'=>'nullable|string|max:255','seo_description'=>'nullable|string|max:1000','spec_keys'=>'nullable|array|max:30','spec_keys.*'=>'nullable|string|max:100','spec_values'=>'nullable|array|max:30','spec_values.*'=>'nullable|string|max:500']);$data['specifications']=collect($data['spec_keys']??[])->map(fn($key,$i)=>['name'=>$key,'value'=>$data['spec_values'][$i]??''])->filter(fn($row)=>filled($row['name']))->values()->all();unset($data['spec_keys'],$data['spec_values']);$data['published_at']=$data['status']==='published'?($material?->published_at??now()):null;return $data;}
}
