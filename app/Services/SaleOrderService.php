...
class SaleOrderService
{
    ...
    {
        if($quantity<1)throw ValidationException::withMessages(["lines.$index.quantity"=>'數量至少為 1。']);$path=parse_url($url,PHP_URL_PATH);$host=strtolower((string)parse_url($url,PHP_URL_HOST));$aliases=$brand->slug==='verdant'?['brand-a.localhost','localhost','127.0.0.1']:($brand->slug==='terracotta'?['brand-b.localhost']:[]);$allowedHosts=array_filter(array_merge([$brand->domain],$aliases));if(!is_string($path)||!in_array($host,$allowedHosts,true))throw ValidationException::withMessages(["lines.$index.url"=>'只接受目前品牌的本站商品連結。']);
